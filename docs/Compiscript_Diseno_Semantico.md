...
Mantener el árbol generado por ANTLR (`grammar/generated/`) fuera del control manual, regenerándolo con `antlr4 -Dlanguage=Python3 Compiscript.g4` mediante `antlr4-tools` (pip) en vez de Docker, evita ediciones accidentales sobre código autogenerado.
...
| `ErrorType` | (interno, no se escribe en el código) | Tipo "comodín" que se devuelve tras un error ya reportado, para no generar errores en cascada sobre el mismo símbolo. |
...
Antes de empezar a codear, conviene que el equipo decida explícitamente estos puntos, que quedan documentados aquí para no perderlos:
