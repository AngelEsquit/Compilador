...
1. **Construir y Ejecutar el Contenedor Docker:** Desde el directorio raiz, ejecuta el siguiente comando para construir la imagen y lanzar un contenedor interactivo:
...
2. **Entender el Entorno**
   ...
   - Este contiene la **gramatica de ANTLR de Compiscript y una version en BNF**, un archivo `Driver.py` (punto de entrada principal) y un archivo `program.cps` (entrada de prueba con la extension de archivos de Compiscript).
3. **Generar Archivos de Lexer y Parser:** Dentro del contenedor, compila la gramatica ANTLR a Python con:
...
4. **Ejecutar el Analizador**
   ...
   - Si el archivo es sintacticamente correcto, **no se mostrara ningun resultado**.
   - Si existen errores, ANTLR los mostrara en la consola.
...
Todos los archivos fuente de Compiscript deben usar la extension:
